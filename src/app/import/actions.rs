...
impl HonkHonk {
    ...
    pub(super) fn import_scanned(&mut self, epoch: u64, mut report: ScanReport) {
        if self.import.epoch != epoch || !self.import.scanning {
            return;
        }
        ...
        self.import.status = format!(
            "{} sounds found. Review selected sounds before importing.",
            report.rows.len()
        );
